...
#else
int mmc_wimax_get_status(void) {return 0;}
#endif
...
void
msmsdcc_reset_and_restore(struct msmsdcc_host *host)
{
	u32	mci_clk = 0;
	u32	mci_mask0 = 0;
	int ret;
	...
	if (!host->clks_on) {
		pr_info("%s: clks are off, skip reset/restore\n",
			mmc_hostname(host->mmc));
		return;
	}
...
msmsdcc_start_command_exec(struct msmsdcc_host *host, u32 arg, u32 c)
{
	 ...
	 */
	if (is_svlte_platform(host->plat) || is_wimax_platform(host->plat))
...
msmsdcc_pio_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if (host->pio.sg == NULL) {
			pr_info("%s: pio sg is null, xfer size %d remain %d\n",
				mmc_hostname(host->mmc),
				host->curr.xfer_size,
		...
		if (host->curr.mrq == NULL) {
			pr_info("%s: curr.mrq is null, xfer size %d	remain %d\n",
				mmc_hostname(host->mmc),
				host->curr.xfer_size,
				host->curr.xfer_remain);
			WARN_ON(1);
...
static int msmsdcc_do_cmdirq(struct msmsdcc_host *host, uint32_t status)
{
	...
	if (!cmd->data || cmd->error) {
		if (host->curr.data && host->dma.sg) {
			...
			msmsdcc_stop_data(host);
			if (cmd->error) {
				pr_info("%s: Command%d err %d\n",
					mmc_hostname(host->mmc), cmd->opcode, cmd->error);
				err = cmd->error;
			}
	...
	do {
		...
		if ((status & (MCI_CMDSENT | MCI_CMDRESPEND | MCI_CMDCRCFAIL |
			...
			if (msmsdcc_do_cmdirq(host, status)) {
				/*
				 * If command error, mrq might be released before irq return
				...
				if (!host->plat->sdiowakeup_irq) {
					...
					(mmc->pm_flags & MMC_PM_WAKE_SDIO_IRQ))
						host->plat->cfg_mpm_sdiowakeup(
							mmc_dev(mmc), 0);
					if(is_wimax_platform(host->plat) || is_svlte_platform(host->plat)) //Wifi doesn't use internal wake up irq
...
static void msmsdcc_req_tout_timer_hdlr(unsigned long data)
{
	...
	for (i = 0; i < host->irq_counter; i++)
		msmsdcc_print_status(host, "irq0-p", host->irq_status[i]);
	spin_lock_irqsave(&host->lock, flags);
	if ((host->plat->dummy52_required) &&
		(host->dummy_52_state == DUMMY_52_STATE_SENT)) {
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->status_irq) {
		set_irq_wake(plat->status_irq, 1);
...
static int msmsdcc_resume(struct device *dev)
{
	...
	if (mmc) {
		 ...
		 */
		if (mmc->card && (mmc->card->type == MMC_TYPE_SDIO) &&
				host->release_sdio_wlock) {
			host->release_sdio_wlock = 0;
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		spin_lock_irqsave(&host->lock, flags);
		if (host->curr.mrq) {
			pr_info("%s: Stop runtime suspend due to new request", mmc_hostname(host->mmc));
			spin_unlock_irqrestore(&host->lock, flags);
...
static int msmsdcc_pm_suspend(struct device *dev)
{
	...
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) 
	{
		printk("%s: %s enter,", mmc_hostname(host->mmc), __func__);
		PRINTRTC;
	}	
	else
		pr_info("%s: %s enter\n", mmc_hostname(host->mmc), __func__);
