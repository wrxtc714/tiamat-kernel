...
enum {
	...
	MSM_MPM_DEBUG_ILDE_BLOCK = BIT(3),
	MSM_MPM_DEBUG_SUSPEND_PENDING_IRQ = BIT(4),
};

static int msm_mpm_debug_mask = MSM_MPM_DEBUG_SUSPEND_PENDING_IRQ;
module_param_named(
...
#ifdef CONFIG_WIMAX
	SDC2_IRQ_0,/*for WiMAX*/
#endif
	SPS_MTI_31,
	INT_DEBUG_TIMER_EXP,
	INT_GP_TIMER_EXP,
	WDT0_ACCSCSSNBARK_INT,
	MARM_WDOG_EXPIRED,
...
static bool msm_mpm_irqs_idle_ignore(void)
{
	int ret;

	ret = bitmap_intersects(msm_mpm_enabled_apps_irqs, msm_mpm_idle_apps_irqs, MSM_MPM_NR_APPS_IRQS);
...
bool msm_mpm_gic_irq_enabled(bool from_idle)
{
	unsigned long *apps_irq_bitmap = from_idle ?
			msm_mpm_enabled_apps_irqs : msm_mpm_wake_apps_irqs;
	int gic_irq_enabled = 0;

	gic_irq_enabled = __bitmap_intersects(msm_mpm_gic_irqs_mask, apps_irq_bitmap,
			MSM_MPM_NR_APPS_IRQS);

	if (gic_irq_enabled && (!from_idle ||
		(MSM_MPM_DEBUG_ILDE_BLOCK & msm_mpm_debug_mask))) {
		char buf[DIV_ROUND_UP(MSM_MPM_NR_APPS_IRQS, 32)*9+1];
		DECLARE_BITMAP(msm_mpm_chk_gic_irqs, MSM_MPM_NR_APPS_IRQS);

		__bitmap_and(msm_mpm_chk_gic_irqs, msm_mpm_gic_irqs_mask,
