...
static int msm_pm_debug_mask = MSM_PM_DEBUG_SUSPEND | MSM_PM_DEBUG_SUSPEND_LIMITS
				| MSM_PM_DEBUG_CLOCK | MSM_PM_DEBUG_RPM_TIMESTAMP | MSM_PM_DEBUG_RPM_STAT;
module_param_named(
...
static void set_power_collapse_status(unsigned cpu, unsigned state)
{
	unsigned *status = (unsigned *)MSM_POWER_COLLAPSE_STATUS_BASE + cpu;
	*status = (POWER_COLLAPSE_STATUS_MAGIC | state);
	mb();
}
	...
	struct msm_pm_device *dev, bool from_idle, bool notify_rpm)
{
	...
	set_power_collapse_status(dev->cpu, 13);
...
int msm_pm_idle_prepare(struct cpuidle_device *dev)
{
	...
	for (i = 0; i < dev->state_count; i++) {
		...
		case MSM_PM_SLEEP_MODE_WAIT_FOR_INTERRUPT:
			...
			rs_limits = msm_rpmrs_lowest_limits(mode, latency_us,
				sleep_us, true);
...
int msm_pm_idle_enter(enum msm_pm_sleep_mode sleep_mode)
{
	...
	case MSM_PM_SLEEP_MODE_POWER_COLLAPSE: {
		...
		ret = msm_rpmrs_enter_sleep(
				sleep_delay, msm_pm_idle_rs_limits, true);
		if (!ret) {
...
skip_dump:
	...
	if (allow[MSM_PM_SLEEP_MODE_POWER_COLLAPSE]) {
		...
		rs_limits = msm_rpmrs_lowest_limits(
				MSM_PM_SLEEP_MODE_POWER_COLLAPSE, -1, -1, false);
		...
		if (rs_limits) {
			ret = msm_rpmrs_enter_sleep(
				msm_pm_max_sleep_time, rs_limits, false);
			if (!ret) {
				msm_pm_power_collapse(false);
				msm_rpmrs_exit_sleep(rs_limits, false);
			}
		} else {
...
static int __init msm_pm_init(void)
{
	...
	*pmd = __pmd((virt_to_phys(msm_pm_collapse_exit) & PGDIR_MASK) |
	...
	/* 8x60 projects: suspend console deferred to latter stages */
	suspend_console_deferred = 1;

	*tz_entry_0 = 0;
	*(tz_entry_0+1) = 0xDEADDEAD;
	*tz_exit_0 = 0;
	*(tz_exit_0+1) = 0xDEADDEAD;
	*msm_kernel_exit_0 = 0;
	*(msm_kernel_exit_0+1) = 0xDEADDEAD;
	*msm_kernel_exit_1 = 0;
	*(msm_kernel_exit_1+1) = 0xDEADDEAD;
	*tz_entry_1 = 0;
	*(tz_entry_1+1) = 0xDEADDEAD;
	*tz_exit_1 = 0;
	*(tz_exit_1+1) = 0xDEADDEAD;
	*tz_progress = 0;
	*(tz_progress+1) = 0xDEADDEAD;
	*tz_kernel_addr = 0;
	*(tz_kernel_addr+1) = 0xDEADDEAD;
	*tz_error = 0;
